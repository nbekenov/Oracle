    ...
    begin 
      ...
      -- логируем сколько записей осталось после архивации
        execute immediate 'select count(*) from ma_temp.' ||table_name2|| '' into current_records2;
          ...
          begin
                dbms_stats.gather_table_stats('ma_temp',table_name2,estimate_percent=>10);
...
--    refin_tables_arr varray_varchar;
	...
	-- логируем сколько записей осталось после архивации
      ...
      -- если архивация прошла успешно, то логируем время завершения
               ...
               begin
                for tab in 1..refin_tables_arr.count loop
                 schem:= case WHEN refin_tables_arr(tab) in ('ESP_REFINANCE2' )
                              then 'ODM'
                              else 'MA_TEMP' 
                              end ||''; 
...
begin
  ...
  is_ok := ma_temp.CHECKTYPES('MA_TEMP',table_name, 'MA_TEMP','ARC_'||table_name);
  if is_ok='no' then
	LOGING('problem with check attribute types in table '||table_name);
   --проверка соответствия типов полей архивной и архиивруемой таблицы 
	else  is_ok := MA_TEMP.CHECKATTRIBUTES('MA_TEMP',table_name, 'MA_TEMP','ARC_'||table_name);
  end if;  
  if is_ok='yes' then
...
begin  
  ...
  DROP_TMP_IF_EXIST (  table_name2 => 'MA_TEMP.' || table_name);
    ...
    'create table ma_temp.REFINANCE_INPUT_TMP as
      select * from MA_TEMP.REFINANCE_INPUT
...
raise problem_with_attr;
  ...
  refin_tables_arr  := varray_varchar('ESP_REFINANCE2','REFINANCE_LOG_DECISION_F8', 'PRED_RECALC_OFFER', 'PRED_RECALC_VARIANT'
   ...
   for tab in 1..refin_tables_arr.count loop  
    is_ok := MA_TEMP.CHECKTYPES('MA_TEMP',refin_tables_arr(tab), 'MA_TEMP','ARC_'||refin_tables_arr(tab));
      if is_ok='no' then
        raise problem_with_types;
      end if;
	  is_ok := MA_TEMP.CHECKATTRIBUTES('MA_TEMP',refin_tables_arr(tab), 'MA_TEMP','ARC_'||refin_tables_arr(tab));
      if is_ok='no' then 
        raise problem_with_attr;
      end if;
	end loop;
    ...
    DBMS_OUTPUT.PUT_LINE('proble with attributes in REFIN tables');
    
    when problem_with_types then 
     DBMS_OUTPUT.PUT_LINE('proble with types in REFIN tables');
 end;  

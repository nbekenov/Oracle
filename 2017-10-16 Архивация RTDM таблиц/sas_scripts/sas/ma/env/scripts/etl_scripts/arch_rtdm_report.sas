...
libname MYLIB meta LIBRARY=&MON_LIB;
%let ADMIN_EMAIL ="";
%let USER_EMAIL = "leto_mccm_support@glowbyteconsulting.com";
/*
...
%let max_run_id;
proc sql noprint;
select max(run_id) into :max_run_id from MYLIB.rtdm_arch_history;
quit;

data added_values;
format archived_pct percent6.0;
format delta_minutes 10.2;
set MYLIB.rtdm_arch_history;
...
filename mail email to=(&USER_EMAIL) cc=(&ADMIN_EMAIL) subject="DEV RTDM tables archivation results" content_type="text/html";
ODS LISTING CLOSE;
ODS HTML BODY=mail encoding='wcyrillic';

proc report data=sorted ;
TITLE 'Результаты архивирования таблиц RTDM';
